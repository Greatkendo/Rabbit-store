   ...
   <div className="p-6 rounded-lg mb-6">
    ...
    <form className="flex flex-col justify-center gap-3" onSubmit={handleSubmit}>
     ...
     <div className="mb4">
      ...
      <select name="role" value={formData.role} onChange={handleChange} className="w-full p2 border rounded">
     ...
     <tbody>
      {
       users.map((user) => (
        <tr key={user._id} className="border-b hover:bg-gray-50">
         ...
         <td className="p-4">
          <select name="role" value={user.role} onChange={(e) => handleRoleChange(user._id, e.target.value)} className="p-2 border rounded">
           <option value="customer">Customer</option>
           <option value="admin">Admin</option>
          </select>
         </td>
